(function ($) {
  ...
  /*---------- 01. On Load Function ----------*/
    ...
    //     });
    // }
    let pageLoadTimeout = setTimeout(() => {
        window.location.href = "error.html";
    }, 10000);

    window.addEventListener("load", function () {
        // Page loaded successfully, cancel the redirect timer
        clearTimeout(pageLoadTimeout);

        let textElement = document.getElementById("loadingText");
        let current = 0;
        let target = 100;
        let speed = 5; // ms between increments
        ...
        let interval = setInterval(() => {
            ...
            if (current >= target) {
                ...
                // Fade out preloader after counting is done
                let opacity = 1;
                let fade = setInterval(() => {
                    opacity -= 0.05;
                    document.querySelector(".preloader").style.opacity = opacity;
                    if (opacity <= 0) {
                        clearInterval(fade);
    ...
    if ($('.scroll-top').length > 0) {
        ...
        var whatsappbtn = document.querySelector('.scroll-top');
        ...
        init: function () {
            return this.each(function () {
                ...
                $.each(letters, function (i, letter) {
                    $this.append('<span class="char' + (i + 1) + '">' + letter + '</span>');
                });
            });
        }
    };
